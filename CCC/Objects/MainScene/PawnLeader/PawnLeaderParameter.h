/**
 * @file   PawnLeaderParameter.h
 *
 * @brief  ポーンリーダーオブジェクトクラスのパラメータヘッダファイル
 *
 * @author CatCode
 *
 * @date    2026/01/08
...
namespace PawnLeaderParameter
{
	...
	{
		static constexpr float BROKEN = 0.15f;	// 01.5%未満なら崩壊している
		static constexpr float STABLE = 0.7f;	// 70.0%より上なら安定している

		static constexpr float RECOVERY_TIME = 2.5f;	// 陣形崩壊後の安定度を回復する時間
	}

	// 陣形の設定
	static constexpr int   NUMBER_PAWN          = 30;	// 隊員数
